...
export function RiskManagementTab({
  ...
  const defaultStrategies: StrategyDefinition[] = [
    { id: 'momentum', name: 'Momentum', description: 'Trade based on price momentum signals', riskLevel: 'standard' },
    { id: 'mean-reversion', name: 'Mean Reversion', description: 'Trade reversals when price deviates from mean', riskLevel: 'conservative' },
    { id: 'narrative-follow', name: 'Narrative AI', description: 'Follow trending market narratives with AI', riskLevel: 'degen' },
  ];
  ...
  return (
    <div className="w-full max-w-4xl mx-auto">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white/[0.02] border border-white/[0.06] rounded-xl p-4">
            ...
            <div className="flex flex-wrap gap-1.5 mb-3">
              {Array.from(selectedTokens).slice(0, 6).map(token => (
                <span key={token} className="text-xs bg-tago-yellow-400/20 text-tago-yellow-400 px-2 py-1 rounded">
                  {token}
                </span>
              ))}
              {selectedTokens.size > 6 && (
                <span className="text-xs text-white/40 px-2 py-1">+{selectedTokens.size - 6} more</span>
              )}
            </div>
            <button
                    ...
                    >
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          <span className={`text-[9px] px-1.5 py-0.5 rounded uppercase font-medium ${riskColors[strategy.riskLevel]}`}>
                            {strategy.riskLevel}
                          </span>
                        </div>
                      ...
                      <button
                        ...
                        } ${isTogglingStrategy || !onToggleStrategy ? 'opacity-50' : ''}`}
                      >
                        <span
        ...
        <div className="space-y-4">
          {/* Live Risk Metrics */}
          <div className="bg-gradient-to-br from-white/[0.04] to-transparent border border-white/[0.08] rounded-xl p-4">
      ...
      {showTokenModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
          <div className="bg-black/95 border border-white/[0.1] rounded-2xl w-full max-w-lg max-h-[80vh] overflow-hidden">
            {/* Modal Header */}
