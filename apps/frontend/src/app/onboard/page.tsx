...
export default function OnboardPage() {
  ...
  const loadOptions = useCallback(async () => {
    try {
      ...
      // Filter to only tokens with logos
  ...
  useEffect(() => { setOptionsLoading(true); loadOptions().finally(() => setOptionsLoading(false)); }, []);
  useEffect(() => { const i = setInterval(() => loadOptions(), PRICE_REFRESH_INTERVAL); return () => clearInterval(i); }, [loadOptions]);
  ...
  if (!isConnected) {
    return (
      <div className="min-h-[60vh] flex items-center justify-center px-4">
        <div className="w-full max-w-[380px] bg-gradient-to-br from-white/[0.03] to-white/[0.01] backdrop-blur-xl rounded-2xl border border-white/[0.08] p-8 text-center">
          ...
          <h2 className="text-xl font-light text-white mb-2">Deposit to Hyperliquid</h2>
          <p className="text-sm font-light text-white/40 mb-6">Connect wallet to continue</p>
          <ConnectButton />
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-[60vh] flex items-center justify-center px-4" onClick={() => setSelectorOpen(false)}>
      {/* Toast */}
      {toast && (
        <div className="fixed top-4 right-4 z-50 animate-in slide-in-from-top fade-in duration-200">
      ...
      {/* Main Card */}
      <div className={`w-full max-w-[380px] transition-all duration-500 ${isProcessing ? 'blur-sm pointer-events-none' : ''}`} onClick={e => e.stopPropagation()}>
        <div className="bg-gradient-to-br from-white/[0.03] to-white/[0.01] backdrop-blur-xl rounded-2xl border border-white/[0.08] overflow-hidden">
          {/* Header */}
          <div className="px-5 py-4 flex items-center justify-between border-b border-white/[0.05]">
            <h1 className="text-base font-light text-white">Deposit</h1>
            <ConnectButton.Custom>
              {({ openAccountModal }) => (
                <button onClick={openAccountModal} className="flex items-center gap-2 px-2.5 py-1.5 rounded-lg bg-white/[0.03] hover:bg-white/[0.06] border border-white/[0.05] hover:border-white/[0.10] transition-all duration-200">
                  <div className="w-2 h-2 rounded-full bg-green-400" />
                  <span className="text-xs text-white/60 font-mono">{walletAddress.slice(0, 6)}...{walletAddress.slice(-4)}</span>
                </button>
              )}
            </ConnectButton.Custom>
          </div>
          ...
          <div className="p-5 space-y-4">
            ...
            {/* You Receive */}
            <div>
              <span className="text-xs font-light text-white/40 block mb-2">You receive</span>
              <div className="bg-white/[0.03] rounded-xl border border-white/[0.05] p-4">
            ...
            {!quote ? (
              ...
              <div className="space-y-3">
                ...
                <div className="flex gap-2">
                  <a href="https://app.hyperliquid.xyz/trade" target="_blank" rel="noopener noreferrer" className="flex-1 py-3 bg-gradient-to-r from-yellow-400 to-yellow-500 hover:from-yellow-300 hover:to-yellow-400 text-black text-sm font-medium rounded-xl transition-all duration-200 text-center shadow-lg shadow-yellow-500/20">Trade</a>
                  <button onClick={resetFlow} className="px-4 py-3 bg-white/[0.03] border border-white/[0.05] hover:border-white/[0.10] text-white/70 text-sm font-light rounded-xl hover:bg-white/[0.05] transition-all duration-200">New</button>
                </div>
              </div>
            ) : (
      ...
      {isProcessing && (
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/50 backdrop-blur-md">
          <div className="bg-gradient-to-br from-white/[0.03] to-white/[0.01] backdrop-blur-xl border border-white/[0.08] rounded-2xl p-8 text-center max-w-xs mx-4">
